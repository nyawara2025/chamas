...
export const loginResident = async (phone, memberId) => {
  ...
  const userData = {
    ...
    phone: userResponse.phone_number || userResponse.phone,
 ...
 */
export const getTableBankingSummary = async (phone) => {
  const orgId = getOrgId();
  const params = new URLSearchParams();
  
  // Log it so you can see it in your browser console
  console.log('[apiClient] Fetching summary for:', phone);

  params.append('phone_number', phone || '');
  if (orgId) params.append('org_id', orgId);

  const response = await fetch(API_ENDPOINTS.getTableBankingSummary, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: params.toString(),
  });
  ...
  const text = await response.text();
  if (!text) return { balance: 0, due_date: null, history: [] };

  try {
    return JSON.parse(text);
  } catch (e) {
    console.error('Failed to parse n8n JSON:', text);
    return { balance: 0, due_date: null, history: [] };
  }
};
